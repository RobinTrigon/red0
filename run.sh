...
for k in $(cat tempredirect.txt);do
	echo "$k" | xargs -P30 -n1 curl -I |egrep "google.com|216.58.214.206|172.217.167.46|216.58.214.206|evil.com|gov.ai|example.com" &>/dev/null && echo -e "$k" | tee -a resulttemp.txt
done
clear
banner
...
for k in $(cat tempredirect.txt);do
	echo "$k" | xargs -P30 -n1 curl -s -m5 -I|egrep "google.com|216.58.214.206|172.217.167.46|216.58.214.206|evil.com|gov.ai|example.com" &>/dev/null && echo -e "$k" | tee -a resulttemp.txt
done
clear
banner
